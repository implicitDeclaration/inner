...
class WanetCifar(Dataset):
    def __init__(self,
        ...
        self.data = np.load(os.path.join(root, 'wanet_cifar10.npy'), allow_pickle=True)
        self.targets = np.load(os.path.join(root, 'wanet_cifar10_label.npy'))
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'ptest' and not self.return_true_label:
            target = self.trigger_label

        return img, target


class SSBACifar(CIFAR10):
    base_folder = 'cifar-10-batches-py'
    ...
    def __init__(self,
        ...
        super(SSBACifar, self).__init__(root, train, transform, target_transform, download)
        self.return_true_label = return_true_label
        self.mode = mode
        self.trigger_label = trigger_label
        self.transform = transform
        self.target_transform = target_transform
        if train:
            self.data = np.load(os.path.join(root, 'ssba/cifar10_ssba_train_b1.npy'))
        else:
            self.data = np.load(os.path.join(root, 'ssba/cifar10_ssba_test_b1.npy'))
...
class PoisonedCifarPair(CIFAR10): # return image pairs of bd and clean
    base_folder = 'cifar-10-batches-py'
    url = "https://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz"
    filename = "cifar-10-python.tar.gz"
    tgz_md5 = 'c58f30108f718f92721af3b95e74349a'
    ...
    def __init__(self,
        ...
        super(PoisonedCifarPair, self).__init__(root, train, transform, target_transform, download)
        self.return_true_label = return_true_label
        self.trigger_label = trigger_label
        self.p_rate = p_rate
        # have three different mode "train" "ptest" "test"
...
class WanetGTSRB(Dataset):
    def __init__(self, root, train=True,
        ...
        self.data = np.load(os.path.join(root, 'wanet_gtsrb.npy'), allow_pickle=True) # ***
        self.targets = np.load(os.path.join(root, 'wanet_gtsrb_label.npy'))
...
class SSBAGTSRB(Dataset):
    def __init__(self, root, train=True,
        ...
        self.target_transform = target_transform
        if train:
            self.data = np.load(os.path.join(root, 'ssba/gtsrb_ssba_train_b1.npy'))
            csv_path = os.path.join(root, "Train.csv")
        else:
            self.data = np.load(os.path.join(root, 'ssba/gtsrb_ssba_test_b1.npy'))
            csv_path = os.path.join(root, "Test.csv")
        ...
        # # data selection, remove those clean samples with trigger label
        if avoid_trg_class:
            targets = np.array(self.class_ids)
            nontrigger_idx = np.where(targets != self.trigger_label)
            self.data = self.data[nontrigger_idx]
            self.class_ids = targets[nontrigger_idx]
