...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        work_dir = osp.join(self.current_schedule['save_dir'],
                            self.current_schedule['experiment_name'] + '_' + time.strftime("%Y-%m-%d",
                                                                                           time.localtime()))
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            for batch_id, batch in enumerate(train_loader):
                ...
                if iteration % self.current_schedule['log_iteration_interval'] == 0:
                    msg = time.strftime("[%Y-%m-%d_%H:%M:%S] ",
                                        time.localtime()) + f"Epoch:{i + 1}/{self.current_schedule['epochs']}, iteration:{batch_id + 1}/{len(self.poisoned_train_dataset) // self.current_schedule['batch_size']}, lr: {self.current_schedule['lr']}, loss: {float(loss)}, time: {time.time() - last_time}\n"
                    last_time = time.time()
                    log(msg)
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                predict_digits, labels = self._test(self.poisoned_test_dataset, device,
